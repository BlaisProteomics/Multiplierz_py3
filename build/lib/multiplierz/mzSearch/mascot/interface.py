...
def MascotCSVToMultiplierzReport(csv_data, outputfile = None):        
    ...
    for line in csv_data:
        ...
        if mods or mod_loc_str:
            ...
            # representation.
            # 2021-08-09-Several bug fixes to previous version
            ...
            if mod_loc_str.split('.')[0] != '0':
                modname = varmod_numbering[int(mod_loc_str.split('.')[0])]
                mod_strs.append("Nterm: %s" % modname)
            if mod_loc_str.split('.')[-1] != '0':
                modname = varmod_numbering[int(mod_loc_str.split('.')[-1])]
